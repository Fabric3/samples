 ...
 * 
 */
@WebServiceClient(name = "LoanServiceService", targetNamespace = "http://loan.api.bigbank.samples.fabric3.org/", wsdlLocation = "http://localhost:8181/wsloan?wsdl")
public class LoanServiceService
    extends Service
{

    private final static URL LOANSERVICESERVICE_WSDL_LOCATION;
    private final static Logger logger = Logger.getLogger(org.fabric3.samples.bigbank.api.loan.LoanServiceService.class.getName());

    static {
        URL url = null;
        try {
            URL baseUrl;
            baseUrl = org.fabric3.samples.bigbank.api.loan.LoanServiceService.class.getResource(".");
            url = new URL(baseUrl, "http://localhost:8181/wsloan?wsdl");
        } catch (MalformedURLException e) {
            logger.warning("Failed to create URL for the wsdl Location: 'http://localhost:8181/wsloan?wsdl', retrying as a local file");
